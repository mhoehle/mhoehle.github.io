...
suppressPackageStartupMessages(library(Matrix)) #Sparse matrix etc.
...
The [R-markdown source code of this blog](`r str_c("https://raw.githubusercontent.com/mhoehle/mhoehle.github.io/master/_source/",current_input())`) is available under a [GNU General Public License (GPL v3)](https://www.gnu.org/licenses/gpl-3.0.html) license from GitHub.
...
```{r, echo=FALSE}
# Build the transition matrix of the Markov chain for all transitions of an A vs. D attack
#
# @param A Number of attacking armies
# @param D Number of defending armies
# @verbose Print additional information, e.g. a progress indicator
#
# @return Sparse (A+1)(D+1) \times (A+1)(D+1) transition matrix 
...
transition_matrix <- function(A, D, verbose=FALSE) {
  ...
  for (i in 1:nrow(states)) {
    # Calculate p_{ij} 
    if (verbose & (i %% round(0.1*nrow(states)) == 0)) {
      cat(str_c(round(100*(i / nrow(states)),digits=0), "% "))
    }
    ...
    if (from_attacker==0 | from_defender==0) {
      ...
      P[i, idx_set] <- to_state %>% pull(prob)
    }
  }
...
# Helper function to calculate matrix power, i.e. X^n, see answer in  https://stackoverflow.com/questions/3274818/matrix-power-in-r
matpow <- function(X, n) Reduce(`%*%`, replicate(n, X, simplify = FALSE))

# Compute the transition matrix for the given A and D
P <- transition_matrix(A=A, D=D)
P_times_AD <- matpow(P, A+D)
```
...
```
```{r x_AD_print, echo=FALSE}
round(x_AD[, as.logical(x_AD>0)], digits=3)
```
...
ggplot(tab_long, aes(x=D, y=A, fill=as.factor(prob))) +
  ...
  geom_vline(data=tibble(A=c(15,25,35)), aes(xintercept=A), lty=3, col="steelblue") +
  coord_equal() +
  scale_y_continuous(minor_breaks = seq(0,D,by=1)) +
  scale_x_continuous(minor_breaks = seq(0,A,by=1)) +
...
```{r AS_FUNC_OF_A, echo=FALSE}
# Show as a function in A when D is fixed
tab_df <- tab %>% select(A, `15`,`25`,`35`) %>% 
  pivot_longer(cols=-A, values_to="prob", names_to="D") %>% 
  mutate(D = as.numeric(D)) 

# First A above 50%
first_A_gt50 <- tab_df %>% group_by(D) %>% summarise(min_A = min(A[prob>0.5]))
first_A <- left_join(first_A_gt50, tab_df, by=c("D", "min_A"="A")) %>% 
  rename(A=min_A)

# Show the graphs
ggplot(tab_df, aes(x=A, y=prob)) + 
  scale_x_continuous(breaks=seq(0,A, by=5), minor_breaks = seq(0,A,by=1)) +
  geom_line() +
  geom_hline(yintercept=0.5, col="lightgreen", lty=2) +
  geom_linerange(data=first_A, aes(ymin=0, ymax=prob), col="lightgreen", lty=2) +
  ylab("P(Attacker wins)") +
  xlab("Attacking armies") +
...
The vertical line in each plot shows the smallest number of attacking armies, s.t. the probability of the attacker winning is greater than 50%. The values are `r str_c(first_A %>% pull(A), collapse=",")`, respectively.
...
The support of $\Delta$ is $\{-D,\ldots,-1,1,\ldots,A\}$; below is shown the corresponding probability mass function (PMF) of $\Delta$ for the `r A` vs. `r D` battle.

```{r PLOT_DIFF, echo=FALSE}
# Start state
pi_start <- (states$state == sprintf("%02d-%02d", A, D))
P_times_AD <- matpow(P, A+D)
pi_end <- pi_start %*% P_times_AD

# Fetch prob of all absorbing states.
absorbing <- states %>% filter(attacker==0 | defender==0) %>% 
  # Make additional info
  mutate(Delta = attacker - defender, 
         W_wins = Delta > 0, 
         L = pmax(0,Delta))

df_Delta <- left_join(absorbing, tibble(state=colnames(pi_end), prob=as.numeric(pi_end)), by="state")

# Show PMF
ggplot(df_Delta) + geom_linerange(aes(x=Delta, ymin=0, ymax=prob, color=W_wins)) +
  xlab(expression(delta)) +
  ylab(expression(P(Delta==delta))) +
  scale_color_discrete(name="Attacker wins") +
  theme(legend.position="bottom")

## Probability that attacker wins
p_A_wins <- df_Delta %>% filter(L>0) %>% summarise(prob=sum(prob)) %>% as.numeric()

# checksame probability in the earlier table
if (FALSE) tab %>% filter(A==50) %>% select(`50`) 

## Conditional expectation of armies lost given that A wins
E_loss_given_A_wins <- df_Delta %>% 
  # Determine conditional probability
  filter(L>0) %>% 
  mutate(prob = prob/sum(prob)) %>% 
  # Compute E(L|L>0)
  mutate(E_contrib = L*prob) %>% 
  summarise(E=A - sum(E_contrib)) %>% 
  as.numeric()
```

Note that if $\Delta>0$, then the attacker wins. Hence, for better visualization, the plot of this sawtooth shaped PMF is colored using the two outcomes of the battle. The overall probability for the attacker to win is `r sprintf("%.1f", 100*p_A_wins)`%, which can be found as the sum of the cyan bars. If focus is restricted to the number of armies the attacker loses during the attack, one could instead consider $L:=\max(0,\Delta)$, which corresponds to stacking all red bars of the PMF in the value 0, while keeping the cyan part. The expected number of armies the attacker looses in a battle won is then $A-\operatorname{E}(L|L>0)$, which for the given `r A` vs. `r D` battle means that the attacker is expected to lose `r sprintf("%.1f", E_loss_given_A_wins)` armies when winning.
...
$$
where the last equality follows from properties of the [fundamental matrix](https://en.wikipedia.org/wiki/Absorbing_Markov_chain#Fundamental_matrix) of the absorbing Markov chain [@grinstead_snell1997, Thm. 11.4]. The statement of the above equation corresponds to Theorem 11.6. in @grinstead_snell1997.
